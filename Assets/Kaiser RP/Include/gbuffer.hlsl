...
GbufferData unpack(GbufferDataPacked data)
{
    GbufferData res;
    res.albedo = data.unpack_albedo();
    res.normal = data.unpack_normal();

    float2 roughness_metalness = unpack_2x16f_uint(data.data0.z);
    res.roughness = perceptual_roughness_to_roughness(roughness_metalness.x);
    res.metalness = roughness_metalness.y;
    res.emissive = data.unpack_emissive();
